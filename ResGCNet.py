    ...
    for stack in range(3):
        for res_block in range(num_res_blocks):
            ...
            x = Activation('relu')(x)
             ...
             # must be layer type(using Lambda(func)(x) or Lambda(lambda x:fuc))
        num_filters *= 2
...
save_dir = 'mytensorboard/res_non_local_net56-v2'
model_name = 'cifar10_%s_.{epoch:03d}_.{val_acc}.h5' % model_type
...
earlystop=EarlyStopping(monitor='val_acc',patience=30)
...
callbacks = [checkpoint, lr_reducer, lr_scheduler, tensorboard, earlystop]
